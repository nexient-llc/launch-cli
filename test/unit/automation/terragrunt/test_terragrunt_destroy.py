...
def test_terragrunt_destroy_run_all(mock_run):
    ...
    mock_run.assert_called_once_with(['terragrunt', 'run_all', 'destroy', '-auto-approve', '--terragrunt-non-interactive'], check=True)
...
def test_terragrunt_destroy_no_run_all(mock_run):
    ...
    mock_run.assert_called_once_with(['terragrunt', 'destroy', '-auto-approve', '--terragrunt-non-interactive'], check=True)
...
def test_terragrunt_destroy_with_file(mock_run):
    ...
    mock_run.assert_called_once_with(['terragrunt', 'destroy', '-auto-approve', '--terragrunt-non-interactive', '-var-file', 'vars.tfvars'], check=True)
