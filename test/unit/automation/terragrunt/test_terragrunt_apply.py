...
def test_terragrunt_apply_run_all(mock_run):
    ...
    mock_run.assert_called_once_with(['terragrunt', 'run_all', 'apply', '-auto-approve', '--terragrunt-non-interactive'], check=True)
...
def test_terragrunt_apply_no_run_all(mock_run):
    ...
    mock_run.assert_called_once_with(['terragrunt', 'apply', '-auto-approve', '--terragrunt-non-interactive'], check=True)
...
def test_terragrunt_apply_with_file(mock_run):
    ...
    mock_run.assert_called_once_with(['terragrunt', 'apply', '-auto-approve', '--terragrunt-non-interactive', '-var-file', 'vars.tfvars'], check=True)
