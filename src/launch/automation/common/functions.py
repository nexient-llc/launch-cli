...
def deploy_remote_state(
    ...
    if provider_config['az'].get('storage_account_name'):
        run_list.append(f"STORAGE_ACCOUNT_NAME={provider_config['az'].get('storage_account_name')}")
    if provider_config['az'].get('resource_group_name'):
        run_list.append(f"RESOURCE_GROUP_NAME={provider_config['az'].get('resource_group_name')}")
